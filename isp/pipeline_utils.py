...
from .lens_correction import vignetting_correction
# import lensfunpy
...
def get_metadata(image_path):
    ...
    #crop region
    metadata['crop_region'] = get_crop_region(tags, ifds)
    metadata['crop_size'] = get_crop_size(tags, ifds)
...
def get_crop_region(tags, ifds):
    possible_keys = ['Image Tag 0xC61E', 'Image Tag 50719', 'DefaultCropOrigin']
    vals = get_values(tags, possible_keys)
    if vals is None:
        print("Noise profile not found in exifread tags. Searching IFDs.")
        vals = get_tag_values_from_ifds(50719, ifds)
    return vals
def get_crop_size(tags, ifds):
    possible_keys = ['Image Tag 0xC620', 'Image Tag 50720', 'DefaultCropSize']
    vals = get_values(tags, possible_keys)
    if vals is None:
        print("Noise profile not found in exifread tags. Searching IFDs.")
        vals = get_tag_values_from_ifds(50720, ifds)
    return vals
...
def init_tone_map():
    ...
    for y in range(1454):
        length = 0
        for x in range(1218):
            if curve[x, y, 0] == 189 and curve[x, y, 1] == 114 and curve[x, y, 2] == 0:
                tempx = x
                while tempx < 1217 and (curve[tempx + 1, y, 0] != 255 and curve[tempx + 1, y, 1] != 255 and curve[
                    tempx + 1, y, 2] != 255):
                    length += 1
                    tempx += 1
                result[y] = 1218 - (x + length // 2)
                break
    x = np.arange(0, 1454) / 1454
...
def apply_tone_map(x, tone_fn):
    ...
    #tone_mapped_img = tone_mapped_img / acesfilm(1)
...
def apply_lens_correction(normalized_image):


    lens_corrected_image = vignetting_correction(normalized_image * 255)
    return lens_corrected_image / 255.0
...
def add_caliberate_noise(raw_image, noise_profile, ratio, p_r, p_g, p_b):
    ...
    r_read = float(noise_profile[0])
    r_noise = np.random.randn(raw_image.shape[0], raw_image.shape[1]) * np.sqrt(r_read) + np.random.poisson(lam = np.clip(raw_image * p_r(raw_image), 0, 1), size=(raw_image.shape[0], raw_image.shape[1]))



    g_read = float(noise_profile[1])
    g_noise = np.random.randn(raw_image.shape[0], raw_image.shape[1]) * np.sqrt(g_read) + np.random.poisson(lam = np.clip(raw_image * p_g(raw_image), 0, 1), size=(raw_image.shape[0], raw_image.shape[1]))



    b_read = float(noise_profile[2])
    b_noise = np.random.randn(raw_image.shape[0], raw_image.shape[1]) * np.sqrt(b_read) + np.random.poisson(lam = np.clip(raw_image * p_b(raw_image), 0, 1), size=(raw_image.shape[0], raw_image.shape[1]))
    ...
    return np.clip(result, 0, 1), 1, g_read


def add_noise(raw_image, noise_profile, as_shot_neutral, ratio, middle_ratio, low_ratio):
    ...
    # g_noise = torch.from_numpy(g_noise).type(torch.FloatTensor).unsqueeze(0).unsqueeze(0)
    # low, high = xfm(g_noise)
    #
    # low = low * low_ratio
    # high[0][:, :, :, :, :] = 0
    # #high[0][:, :, 2, :, :] = 0
    #
    # high[1][:, :, 0, :, :] = high[1][:, :, 0, :, :] * middle_ratio
    # high[1][:, :, 1, :, :] = high[1][:, :, 1, :, :] * middle_ratio
    # high[1][:, :, 2, :, :] = high[1][:, :, 2, :, :] * middle_ratio
    #
    #
    #
    # high[2][:, :, 0, :, :] =  high[2][:, :, 0, :, :]
    ...
    #
    #
    # g_noise = ifm((low, high))[0, 0, :, :].numpy()
...
def add_gray_noise(raw_image, noise_profile, as_shot_neutral, ratio, middle_ratio, low_ratio):
    xfm = DWTForward(J=3, mode='zero', wave='db3')
    ifm = DWTInverse(mode='zero', wave='db3')
    ...
    result[0:h:2, 0:w:2] = raw_image[0:h:2, 0:w:2] + g_noise[ 0:h:2, 1:w:2]
    result[0:h:2, 1:w:2] = raw_image[0:h:2, 1:w:2] + g_noise[ 0:h:2, 1:w:2]
    result[1:h:2, 0:w:2] = raw_image[1:h:2, 0:w:2] + g_noise[ 0:h:2, 1:w:2]
    result[1:h:2, 1:w:2] = raw_image[1:h:2, 1:w:2] + g_noise[ 0:h:2, 1:w:2]
