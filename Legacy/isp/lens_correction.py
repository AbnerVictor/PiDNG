...
def vignetting_correction(im):
    ...
    imgrayr = np.round(imgray)
    idx = np.where(imgrayr >= 250)
    boolean = np.zeros((imgray.shape[0], imgray.shape[1]), dtype = np.uint8)
    boolean[idx] = 255
    num, labels = cv2.connectedComponents(boolean)
    count = 0
    a = []
    for i in range(1, num):
        a.append(len(np.where(labels==i)[0]))
        if len(np.where(labels==i)[0]) > imgray.shape[0] * imgray.shape[1] / 900:
            count+=1
    ...
    if count == 1:
        maxs = 0
        row_step = imgray.shape[0] // 3
        col_step = imgray.shape[1] // 4
        for i in range(0, imgray.shape[0]-row_step, row_step):
            for j in range(0, imgray.shape[1]-col_step, col_step):
                if np.mean(imgray[i:i+row_step, j:j+col_step]) > maxs:
                    row_idx = i
                    col_idx = j
                    maxs = np.mean(imgray[i:i+row_step, j:j+col_step])
        cm_x = sum(j * imgray[i, j] for i in range(row_idx, row_idx + row_step) for j in range(col_idx, col_idx + col_step)
                   ) / sum(imgray[i, j] for i in range(row_idx, row_idx + row_step) for j in range(col_idx, col_idx + col_step))
        cm_y = sum(i * imgray[i, j] for i in range(row_idx, row_idx + row_step) for j in range(col_idx, col_idx + col_step)
                   ) / sum(imgray[i, j] for i in range(row_idx, row_idx + row_step) for j in range(col_idx, col_idx + col_step))
